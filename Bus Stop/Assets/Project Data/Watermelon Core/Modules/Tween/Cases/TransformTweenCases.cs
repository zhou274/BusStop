...
namespace Watermelon
{
    public static class TransformTweenCases
    {
        ...
        /// </summary>
        public static TweenCase DOBezierMove(this Component tweenObject, Vector3 resultValue, float upOffset, float rightOffset, float time, float delay = 0, bool unscaledTime = false, UpdateMethod tweenType = UpdateMethod.Update)
        {
            return new BezierPosition(tweenObject.transform, resultValue, upOffset, rightOffset).SetDelay(delay).SetDuration(time).SetUnscaledMode(unscaledTime).SetUpdateMethod(tweenType).StartTween();
        }
